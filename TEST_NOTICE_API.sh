...
# 공지사항 API 테스트 스크립트 (기존 PostController 사용)
...
curl -s -X GET "$BASE_URL?category=NOTICE&page=0&size=5" | jq '.'
...
# 3. 공지사항 검색 (인증 없이)
echo "3. 공지사항 검색 (인증 없이)"
curl -s -X GET "$BASE_URL?category=NOTICE&search=서비스&page=0&size=5" | jq '.'

echo -e "\n"

# 4. 공지사항 정렬 (최신순)
echo "4. 공지사항 정렬 (최신순)"
curl -s -X GET "$BASE_URL?category=NOTICE&sortBy=createdAt&sortOrder=desc&page=0&size=5" | jq '.'

echo -e "\n"

# 5. 관리자 로그인 (JWT 토큰 획득)
echo "5. 관리자 로그인 (JWT 토큰 획득)"
...
# 8. 일반 사용자 로그인 (권한 없음 테스트)
echo "8. 일반 사용자 로그인 (권한 없음 테스트)"
...
if [ "$USER_JWT_TOKEN" != "null" ] && [ "$USER_JWT_TOKEN" != "" ]; then
    ...
    # 9. 일반 사용자가 공지사항 작성 시도 (권한 없음)
    echo "9. 일반 사용자가 공지사항 작성 시도 (권한 없음)"
    curl -s -X POST "$BASE_URL" \
      -H "Content-Type: application/json" \
...
# 10. 공지사항 삭제 (관리자 권한 필요)
echo "10. 공지사항 삭제 (관리자 권한 필요)"
...
# 11. 최종 공지사항 목록 확인
echo "11. 최종 공지사항 목록 확인"
curl -s -X GET "$BASE_URL?category=NOTICE&page=0&size=10" | jq '.'
